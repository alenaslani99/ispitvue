<template>
    <div class="container">
        ...
        <div class="row">
            <div class="col-lg-4">
                <text-input @keyup="validate('name')" v-model="editData.name.value" id="name" label="Song name" :error="editData.name.errormsg"/>
            </div>
            <div class="col-lg-4">
                <text-input @keyup="validate('price')" v-model="editData.price.value" id="price" label="Price" :error="editData.price.errormsg"/>
            </div>
            <div class="col-lg-4">
                <text-input @keyup="validate('image')" v-model="editData.image.value" id="img" label="Image" placeholder="Image URL" :error="editData.image.errormsg"/>
            </div>
        </div>
        ...
        <div class="row">
            ...
            <p class="text-danger" v-if="editData.selectedGenres.errormsg">{{ editData.selectedGenres.errormsg }}</p>
        </div>
        <div class="row m-4">
            <button @click.prevent="edit" type="submit" class="btn btn-primary btn-lg btn-block">Edit song</button>
...
export default {
    ...
    methods:{
        one(){
            this.song = this.$store.getters["songs/getOneSong"](this.$route.params.id)
            ...
            this.editData.selectedGenres.value = this.song.genre_ids.split(',').map(Number);
        },
        async edit(){
            if(!this.error){
                const dataforsend = {
            ...
            // console.log(dataforsend);
                try{
                    ...
                    if(error.status == 403){
                            ...
                            this.editData.selectedGenres.errormsg = error.response.data.errors['genres'];
                        }else{
                            this.msg = true;
            ...
            switch(type){
                case "name":
                    if(this.editData.name.value.length < 3){
                        this.error = true;
                        this.editData.name.errormsg = "The name must be longer than 3 characters.";
                    }else{
                        this.error = false;
