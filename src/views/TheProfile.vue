        ...
        <div v-if="isArtist" class="row">
            ...
            <div class="col-lg-12">
                ...
                <tbody>
                    ...
                    <th scope="row">{{ i+1 }}</th>
                    <td><img :src="song.image" alt="nesto" class="img-thumbnail"></td>
                    <td>{{ song.name }}</td>
                    <td>$ {{ song.latest_price.price }}</td>
                    <td>{{ song.liked_by_users_count }}</td>
                    <td><router-link :to="'/edit/' + song.id" class="text-primary">Edit</router-link></td>
                    <td><a @click="deleteSong(song.id)" class="text-danger">Delete</a></td>
                    </tr>
                </tbody>
                </table>
            </div>
...
export default {
    ...
    async created(){
        await this.$store.dispatch("auth/User",this.$store.getters['auth/getUserId']);
